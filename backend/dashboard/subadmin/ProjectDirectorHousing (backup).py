...
class ProjectDirectorHousingAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for mat_flag in SystemStatus.objects.filter(system_code = "system_status_task_status_material").first().get_descendants(include_self = False).exclude(system_code="system_status_task_status_material_green"):
            ...
            material_cleared_qs = rectified_qs.filter(status=mat_flag)
            ...
            if 'project__city_id' in request.GET:
               data_dict['url'] = "/dashboard/taskactionlist/?status_id={flag_id}&project__city_id={city_id}".format(flag_id = mat_flag.id,city_id = str(request.GET['project__city_id']))
            elif 'project__region_id' in request.GET:   
                data_dict['url'] = "/dashboard/taskactionlist/?status_id={flag_id}&project__region_id={region_id}".format(flag_id = mat_flag.id,region_id = str(request.GET['project__region_id']))
            else:
        ...
        for ins_flag in SystemStatus.objects.filter(system_code = "system_status_task_status_inspection").first().get_descendants(include_self = False).exclude(system_code="system_status_task_status_inspection_green"):
            ...
            inspection_cleared_qs = rectified_qs.filter(status=ins_flag)
            ...
            if 'project__city_id' in request.GET:
               data_dict['url'] = "/taskactionlist/?status_id={flag_id}&project__city_id={city_id}".format(flag_id = ins_flag.id,city_id = str(request.GET['project__city_id']))
            elif 'project__region_id' in request.GET:   
                data_dict['url'] = "/taskactionlist/?status_id={flag_id}&project__region_id={region_id}".format(flag_id = ins_flag.id,region_id = str(request.GET['project__region_id']))
            else:
        ...
        # HSE Flag Card Filter
        # try:
        #     hse_flags_counts = []
        #     hse_flags_counts_raised = 0
        #     hse_flags_counts_cleared = 0
        #     hse_flags_counts_balance = 0
        #     for hse_flag in SystemStatus.objects.filter(system_code = "system_status_task_status_hse").first().get_descendants(include_self = False).exclude(system_code="system_status_task_status_hse_green"):
        #         hse_qs = qs.filter(status=hse_flag)
        #         hse_pending_qs = hse_qs
        #         hse_cleared_qs = rectified_qs.filter(parent__id__in = hse_pending_qs)

        #         if hse_pending_qs.exists() and hse_cleared_qs.exists():
        #             hse_raised_qs = hse_pending_qs.union(hse_cleared_qs)
        #         else:
        #             hse_raised_qs = hse_pending_qs or hse_cleared_qs

        #         hse_flags_counts_raised = hse_flags_counts_raised + hse_raised_qs.count()
        #         hse_flags_counts_cleared =   hse_flags_counts_cleared + hse_cleared_qs.count()
        #         hse_flags_counts_balance =   hse_flags_counts_balance + hse_pending_qs.count()
        #         data_dict = {
        #             "id":str(hse_flag.id),
        #             "flag_color":hse_flag.color,
        #             "raised_count":hse_raised_qs.count(),
        #             "cleared_count":hse_cleared_qs.count(),
        #             "pending_count":hse_pending_qs.count()
        #         }

        #         if 'project__city_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?hse_flag_id={flag_id}&project__city_id={city_id}".format(flag_id = hse_flag.id,city_id = str(request.GET['project__city_id']))
        #         elif 'project__region_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?hse_flag_id={flag_id}&project__region_id={region_id}".format(flag_id = hse_flag.id,region_id = str(request.GET['project__region_id']))
        #         else:
        #             data_dict['url'] = "/taskactionlist/?hse_flag_id={flag_id}".format(flag_id = hse_flag.id)
        #         hse_flags_counts.append(data_dict)
        #     extra_context['hse_flag_counts'] = list(hse_flags_counts)
        #     extra_context['hse_flags_counts_raised'] = hse_flags_counts_raised
        #     extra_context['hse_flags_counts_cleared'] = hse_flags_counts_cleared
        #     extra_context['hse_flags_counts_balance'] = hse_flags_counts_balance
        # except AttributeError:
        #     print("System status with system code 'system_status_task_status_hse' does not exist.")


        # try:
        #     # SCM Flag Card Filter
        #     scm_flags_counts = []
        #     scm_flags_counts_raised = 0
        #     scm_flags_counts_cleared = 0
        #     scm_flags_counts_balance = 0
        #     for scm_flag in SystemStatus.objects.filter(
        #             system_code="system_status_task_status_scm").first().get_descendants(include_self=False).exclude(
        #             system_code="system_status_task_status_scm_green"):
        #         scm_qs = qs.filter(status=scm_flag)
        #         scm_pending_qs = scm_qs
        #         scm_cleared_qs = rectified_qs.filter(parent__id__in=scm_pending_qs)

        #         if scm_pending_qs.exists() and scm_cleared_qs.exists():
        #             scm_raised_qs = scm_pending_qs.union(scm_cleared_qs)
        #         else:
        #             scm_raised_qs = scm_pending_qs or scm_cleared_qs

        #         scm_flags_counts_raised += scm_raised_qs.count()
        #         scm_flags_counts_cleared += scm_cleared_qs.count()
        #         scm_flags_counts_balance += scm_pending_qs.count()

        #         data_dict = {
        #             "id": str(scm_flag.id),
        #             "flag_color": scm_flag.color,
        #             "raised_count": scm_raised_qs.count(),
        #             "cleared_count": scm_cleared_qs.count(),
        #             "pending_count": scm_pending_qs.count()
        #         }

        #         if 'project__city_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?scm_flag_id={flag_id}&project__city_id={city_id}".format(
        #                 flag_id=scm_flag.id, city_id=str(request.GET['project__city_id']))
        #         elif 'project__region_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?scm_flag_id={flag_id}&project__region_id={region_id}".format(
        #                 flag_id=scm_flag.id, region_id=str(request.GET['project__region_id']))
        #         else:
        #             data_dict['url'] = "/taskactionlist/?scm_flag_id={flag_id}".format(flag_id=scm_flag.id)

        #         scm_flags_counts.append(data_dict)

        #     extra_context['scm_flag_counts'] = list(scm_flags_counts)
        #     extra_context['scm_flags_counts_raised'] = scm_flags_counts_raised
        #     extra_context['scm_flags_counts_cleared'] = scm_flags_counts_cleared
        #     extra_context['scm_flags_counts_balance'] = scm_flags_counts_balance

        # except AttributeError:
        #     print("System status with system code 'system_status_task_status_scm' does not exist.")

        # try:
        #     # Gen Insp Flag Card Filter
        #     gen_insp_flags_counts = []
        #     gen_insp_flags_counts_raised = 0
        #     gen_insp_flags_counts_cleared = 0
        #     gen_insp_flags_counts_balance = 0
        #     for gen_insp_flag in SystemStatus.objects.filter(
        #             system_code="system_status_task_status_gen_insp").first().get_descendants(include_self=False).exclude(
        #             system_code="system_status_task_status_gen_insp_green"):
        #         gen_insp_qs = qs.filter(status=gen_insp_flag)
        #         gen_insp_pending_qs = gen_insp_qs
        #         gen_insp_cleared_qs = rectified_qs.filter(parent__id__in=gen_insp_pending_qs)

        #         if gen_insp_pending_qs.exists() and gen_insp_cleared_qs.exists():
        #             gen_insp_raised_qs = gen_insp_pending_qs.union(gen_insp_cleared_qs)
        #         else:
        #             gen_insp_raised_qs = gen_insp_pending_qs or gen_insp_cleared_qs

        #         gen_insp_flags_counts_raised += gen_insp_raised_qs.count()
        #         gen_insp_flags_counts_cleared += gen_insp_cleared_qs.count()
        #         gen_insp_flags_counts_balance += gen_insp_pending_qs.count()

        #         data_dict = {
        #             "id": str(gen_insp_flag.id),
        #             "flag_color": gen_insp_flag.color,
        #             "raised_count": gen_insp_raised_qs.count(),
        #             "cleared_count": gen_insp_cleared_qs.count(),
        #             "pending_count": gen_insp_pending_qs.count()
        #         }

        #         if 'project__city_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?gen_insp_flag_id={flag_id}&project__city_id={city_id}".format(
        #                 flag_id=gen_insp_flag.id, city_id=str(request.GET['project__city_id']))
        #         elif 'project__region_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?gen_insp_flag_id={flag_id}&project__region_id={region_id}".format(
        #                 flag_id=gen_insp_flag.id, region_id=str(request.GET['project__region_id']))
        #         else:
        #             data_dict['url'] = "/taskactionlist/?gen_insp_flag_id={flag_id}".format(flag_id=gen_insp_flag.id)

        #         gen_insp_flags_counts.append(data_dict)

        #     extra_context['gen_insp_flag_counts'] = list(gen_insp_flags_counts)
        #     extra_context['gen_insp_flags_counts_raised'] = gen_insp_flags_counts_raised
        #     extra_context['gen_insp_flags_counts_cleared'] = gen_insp_flags_counts_cleared
        #     extra_context['gen_insp_flags_counts_balance'] = gen_insp_flags_counts_balance

        # except AttributeError:
        #     print("System status with system code 'system_status_task_status_gen_insp' does not exist.")

        # try:
        #     # Design Insp Flag Card Filter
        #     design_insp_flags_counts = []
        #     design_insp_flags_counts_raised = 0
        #     design_insp_flags_counts_cleared = 0
        #     design_insp_flags_counts_balance = 0

        #     for design_insp_flag in SystemStatus.objects.filter(
        #             system_code="system_status_task_status_design_insp").first().get_descendants(
        #         include_self=False).exclude(
        #         system_code="system_status_task_status_design_insp_green"):
        #         design_insp_qs = qs.filter(status=design_insp_flag)
        #         design_insp_pending_qs = design_insp_qs
        #         design_insp_cleared_qs = rectified_qs.filter(parent__id__in=design_insp_pending_qs)

        #         if design_insp_pending_qs.exists() and design_insp_cleared_qs.exists():
        #             design_insp_raised_qs = design_insp_pending_qs.union(design_insp_cleared_qs)
        #         else:
        #             design_insp_raised_qs = design_insp_pending_qs or design_insp_cleared_qs

        #         design_insp_flags_counts_raised += design_insp_raised_qs.count()
        #         design_insp_flags_counts_cleared += design_insp_cleared_qs.count()
        #         design_insp_flags_counts_balance += design_insp_pending_qs.count()

        #         data_dict = {
        #             "id": str(design_insp_flag.id),
        #             "flag_color": design_insp_flag.color,
        #             "raised_count": design_insp_raised_qs.count(),
        #             "cleared_count": design_insp_cleared_qs.count(),
        #             "pending_count": design_insp_pending_qs.count()
        #         }

        #         if 'project__city_id' in request.GET:
        #             data_dict['url'] = "/taskactionlist/?design_insp_flag_id={flag_id}&project__city_id={city_id}".format(
        #                 flag_id=design_insp_flag.id, city_id=str(request.GET['project__city_id']))
        #         elif 'project__region_id' in request.GET:
        #             data_dict[
        #                 'url'] = "/taskactionlist/?design_insp_flag_id={flag_id}&project__region_id={region_id}".format(
        #                 flag_id=design_insp_flag.id, region_id=str(request.GET['project__region_id']))
        #         else:
        #             data_dict['url'] = "/taskactionlist/?design_insp_flag_id={flag_id}".format(flag_id=design_insp_flag.id)

        #         design_insp_flags_counts.append(data_dict)

        #     extra_context['design_insp_flag_counts'] = list(design_insp_flags_counts)
        #     extra_context['design_insp_flags_counts_raised'] = design_insp_flags_counts_raised
        #     extra_context['design_insp_flags_counts_cleared'] = design_insp_flags_counts_cleared
        #     extra_context['design_insp_flags_counts_balance'] = design_insp_flags_counts_balance

        # except AttributeError:
        #     print("System status with system code 'system_status_task_status_design_insp' does not exist.")
        ...
        # # Initialize an empty dictionary to hold status colors
        # status_colors_dict = {}

        # # Determine the year to filter by
        # current_year = None
        # if 'drf__created_on__gte' in self.request.GET and 'drf__created_on__lte' in self.request.GET:
        ...
        # # Filter queryset for the last five months
        # qs = qs.filter(created_on__year=current_year, created_on__month__in=last_five_months).order_by('status')

        # for item in qs:
        #     status_name = item.status.name
        #     created_month = item.created_on.month
        ...
        # # Get the distinct statuses present in the queryset
        # distinct_statuses = set(
        #     status for status_counts in status_counts_monthwise.values() for status in status_counts.keys())

        # # Iterate over distinct statuses
        # for status in distinct_statuses:
        #     # Initialize a list to hold the data for the current status
        #     data = []
        #     # Iterate over the last five months to maintain sequence
        #     for month in last_five_months:
        #         # Check if there is any flag data for the current month
        #         if month in status_counts_monthwise:
        #             # Get the count for the current status in the current month
        #             count = status_counts_monthwise[month].get(status, 0)
        #             # Append the count to the data list
        #             data.append(count)
        #     # Append the status data to the series data list
        ...
        # # Pass series_data and colors to your template for rendering the graph
        # extra_context['month_wise_flag_percentage_data_series_data'] = series_data
        # extra_context['month_wise_flag_percentage_data_months'] = [
        #     datetime(current_year, month, 1).strftime('%b %Y') for month in last_five_months]
        # extra_context['month_wise_flag_percentage_data_colors'] = colors

        # # Year Wise Flag Stats with percentage
        # # Get counts for each status
        # # Initialize a defaultdict to hold status counts
        # status_counts_yearwise = defaultdict(lambda: defaultdict(int))

        # # Initialize an empty dictionary to hold status colors
        # status_colors_dict = {}

        # # Iterate over the queryset and increment count for each status name
        # for item in qs:
        #     status_name = item.status.name
        #     created_year = item.created_on.year
        #     status_counts_yearwise[created_year][status_name] += 1

        #     # Extract status colors
        #     status_color = item.status.color
        #     status_colors_dict[status_name] = status_color

        # # Initialize an empty set to store distinct years
        # distinct_years = set()
        # # Iterate over the keys (years) in the status_counts_yearwise defaultdict
        # for year in status_counts_yearwise.keys():
        #     # Add the year to the set
        #     distinct_years.add(year)
        # # Convert the set to a sorted list to maintain the sequence
        ...
        # # Get the distinct statuses present in the queryset
        # distinct_statuses = set(
        #     status for status_counts in status_counts_yearwise.values() for status in status_counts.keys())

        # # Iterate over distinct statuses
        # for status in distinct_statuses:
        #     # Initialize a list to hold the data for the current status
        #     data = []
        #     # Iterate over years to maintain sequence
        #     for year in distinct_years:
        #         # Get the count for the current status in the current year
        #         count = status_counts_yearwise[year].get(status, 0)
        #         # Append the count to the data list
        #         data.append(count)
        #     # Append the status data to the series data list
        #     series_data.append({'name': status, 'data': data})
        ...
        # colors = [status_colors_dict[status] for status in distinct_statuses]
        # # Pass series_data and colors to your template for rendering the graph
        # extra_context['year_wise_flag_percentage_data_series_data'] = series_data
        # extra_context['year_wise_flag_percentage_data_years'] = distinct_years
        # extra_context['year_wise_flag_percentage_data_colors'] = colors

        
        # Subquery to get the count of TaskAction for each Region
        # task_action_count_subquery = TaskAction.objects.filter(
        #     project__region=OuterRef('pk'),
        #     project__region__status='active'
        # ).values('project__region').annotate(count=Count('id')).values('count')

        # # Main query to get Region with TaskAction counts
        ...
        # extra_context['zonal_flag_stats_1'] = zonal_stats_formatted_data

        # Zone Wise Flag Stats
        ...
        # # Iterate over the queryset and increment count for each status name
        # for item in TaskAction.objects.filter(status__isnull=False):
        #     status_name = item.status.name
        #     region_name = item.project.region.name  # Assuming region name is extracted from the queryset
        ...
        # # Initialize a set to store distinct regions
        # distinct_regions = set()

        # # Iterate over the keys (regions) in the status_counts_regionwise defaultdict
        # for region in status_counts_regionwise.keys():
        ...
        # # Iterate over distinct statuses
        # for status in distinct_statuses:
        #     # Initialize a dictionary to hold the data for the current status
        #     status_data = {'name': status.capitalize(), 'data': []}
        #     # Iterate over regions to maintain sequence
        #     for region in distinct_regions:
        #         # Get the count for the current status in the current region
        #         count = status_counts_regionwise[region].get(status, 0)
        #         # Append the count to the status data list
        #         status_data['data'].append(count)
        #     # Append the status data to the series data list
        #     series_data.append(status_data)

        # # Extract colors for each status
        # colors = ['#5c5ced','#32CD32','#FF6347']

        # # Pass series_data, colors, and distinct_regions to your template for rendering the graph
        # extra_context['region_wise_flag_percentage_data_series_data'] = series_data
        # extra_context['region_wise_flag_percentage_data_colors'] = colors
        # extra_context['region_wise_flag_percentage_data_regions'] = distinct_regions
    ...
    #     task_action_manager = TaskActionManager(TaskAction,request)
    #     qs = task_action_manager.get_qs().filter(created_on__year = 2024)
    #     status_counts_regionwise = defaultdict(lambda: defaultdict(int))
    #     status_colors_dict = {}
    ...
    #     # Initialize a set to store distinct regions
    #     distinct_regions = set(status_counts_regionwise.keys())
    #     distinct_regions = sorted(list(distinct_regions))
    ...
    #         'region_wise_flag_percentage_data_regions': distinct_regions
    #     }
    #     return JsonResponse(response_data)
...
#
   ...
   # Zone Wise Report
    ...
    #     # Use select_related for better performance
    #     qs = task_action_manager.get_qs().filter(created_on__year=2024).select_related('status', 'project')
    ...
    #     # Prepare distinct regions and series data
    #     distinct_regions = sorted(status_counts_regionwise.keys())
    #     distinct_statuses = ['raised', 'cleared', 'pending']
    #     series_data = [
    #         {
    #             'name': status.capitalize(),
    ...
    def task_action_first_attachment(self,request):
        if request.method == 'POST':
            ...
            #     child_with_null_end_time = task_action_obj.get_descendants().filter(end_time__isnull=True).first()

...
from dashboard.models import TaskActionDashboard
...
class TaskActionDashboardAdmin(admin.ModelAdmin):
    # change_list_template = 'admin/dashboard/change_list.html'
    #change_form_template = 'admin/dashboard/change_form.html'
    
    change_list_template = 'taskaction_dashboard_change_list.html'
    change_form_template = 'change_form.html'
    list_display_links = None
    # list_display = ('reference_number','project_name', 'get_flags','consultant_name','contractor_name')
    readonly_fields = ('get_flags',)
    show_full_result_count = False
    # list_filter = (RegionFilter,CityFilter,FlagsFilter,InspectionFilter,('created_on',DateRangeFilter))
    list_per_page = 5
    # search_fields = ('code','name', 'consultant_name','contractor_name','status__name', 'start_date', 'end_date')
    # ordering = ('project_name','code','status', 'created_on', 'updated_on')
    ...
    #     return super(TaskActionDashboardAdmin, self).changelist_view(request, extra_context=extra_context)

    # This will help you to disable delete functionaliyt
    def get_queryset(self, request):
        qs = self.model._default_manager.get_queryset()
        # qs = qs.filter(region__in=request.user)
        project_ids_list = filtered_qs_rolebased(request)
        qs = qs.filter(project_id__in = project_ids_list)
    ...
    def getTaskActions(self,request):
        if request.method == 'POST':
            ...
            task_action_qs = TaskAction.objects.filter(project_id = project_id,status_id=status_id)
...
admin.site.register(TaskActionDashboard, TaskActionDashboardAdmin)

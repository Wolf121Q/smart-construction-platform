...
class CityFilter(SimpleListFilter):
    ...
    template = 'task_action_templates/task_action_filter.html'
    ...
    def lookups(self, request, model_admin):
        if request.GET:
            ...
            for key in dict(request.GET):
                if not key in ['q','p']:
                    filters[key] = dict(request.GET)[key][0]
            filters['project__city__isnull'] = False
            filter_q = Q(**filters)
            queryset = model_admin.model.objects.filter(filter_q).order_by('project__city__code').distinct('project__city__code')
            cities =  set([c.project.city for c in queryset])
        else:
        ...
        return [(c.id, '{} {}'.format(c.name, str(model_admin.model.objects.filter(project__city_id = c.id).count()))) for c in cities if c is not None]
    ...
    def lookups(self, request, model_admin):
        if request.GET:
            ...
            for key in dict(request.GET):
                if not key in ['q','p']:
                    filters[key] = dict(request.GET)[key][0]
            filters['project__region__isnull'] = False
            filter_q = Q(**filters)
            queryset = model_admin.model.objects.filter(filter_q).order_by('project__region__code').distinct('project__region__code')
            regions =  set([c.project.region for c in queryset])
        else:
            # statuses = set([c.status for c in model_admin.model.objects.order_by('status__system_code').distinct('status__system_code')])
        ...
        return [(c.id, '{} {}'.format(c.name, str(model_admin.model.objects.filter(project__region_id = c.id).count()))) for c in regions if c is not None]
    ...
    def lookups(self, request, model_admin):
        if request.GET:
            ...
            for key in dict(request.GET):
                if not key in ['q','p']:
                    filters[key] = dict(request.GET)[key][0]
            filters['status__parent__isnull'] = False
            filter_q = Q(**filters)
            queryset = model_admin.model.objects.filter(filter_q).order_by('status__parent__code').distinct('status__parent__code')
            parent_flags =  set([c.status.parent for c in queryset])
        else:
            parent_flags =  set([c.status.parent for c in model_admin.model.objects.filter(status__parent__isnull=False).order_by('status__parent__code').distinct('status__parent__code')])
        
        return [(c.id, '{} {}'.format(c.name,str(model_admin.model.objects.filter(status__parent_id = c.id).count()))) for c in parent_flags if c is not None]
...
class MaterialFlagsFilter(SimpleListFilter):
    ...
    template = 'task_action_templates/task_action_flag_filter.html'
    ...
    def lookups(self, request, model_admin):
        if request.GET:
            ...
            filter_q = Q(**filters)
            queryset = model_admin.model.objects.filter(filter_q).order_by('status__code').distinct('status__code')
            flags =  set([c.status for c in queryset])
        else:
            flags =  set([c.status for c in model_admin.model.objects.filter(status__parent__system_code = "system_status_task_status_material",status__isnull=False).order_by('status__code').distinct('status__code')])
        return [(c.id, '{} {}'.format(c.color,str(model_admin.model.objects.filter(status_id = c.id).count()))) for c in flags if c is not None]
...
class InspectionFlagsFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        if request.GET:
            ...
            flags =  set([c.status for c in model_admin.model.objects.filter(status__parent__system_code = "system_status_task_status_inspection",status__isnull=False).order_by('status__code').distinct('status__code')])
        return [(c.id, '{} {}'.format(c.color,str(model_admin.model.objects.filter(status_id = c.id).count()))) for c in flags if c is not None]
...
class TaskListAdmin(admin.ModelAdmin):
    ...
    ordering = ['-created_on']
    # list_filter = (InspectionFilter,FlagsFilter,RegionFilter,CityFilter)
    list_filter = (InspectionFilter,RegionFilter,CityFilter,MaterialFlagsFilter,InspectionFlagsFilter)
    ...
    def get_obsn(self, obj):
        return mark_safe('<a onclick="DashboardApi.obsnModal(\'{desc}\')" href="javascript:void(0);" class="btn btn-bold btn-outline-primary btn-h-primary fs--outline border-0 btn-a-primary radius-0 px-35 mb-1" data-toggle="tooltip" data-original-title="Flag Detail">View Obsn</a>'.format(
                desc = str(obj.description.html),
        ))
    ...
    def changelist_view(self, request, extra_context=None):
        response = super(TaskListAdmin, self).changelist_view(request, extra_context)
